...
containing frequencies and FFT for 32s around H1 detection of GW150914
...
#transform to frequency domain and save as *.txt
FFT_H1 = []
FFT_H1.append(np.fft.rfftfreq(len(time), dt))
FFT_H1.append(np.fft.rfft(strain_H1))
FFT_H1 = np.array(FFT_H1).T
    
np.savetxt('H1-FFT.txt', FFT_H1)


#Issue with np.savetxt leaving +- instead of -, this block resolves that.
# Read in the file
with open('H1-FFT.txt', 'r') as file :
  filedata = file.read()
# Replace the target string
filedata = filedata.replace('+-', '-')
# Write the file out again
with open('H1-FFT.txt', 'w') as file:
  file.write(filedata)
...
if 0:
    ...
    #plt.plot(strain)
    plt.loglog(FFT_H1[1], FFT_H1[0],'r',label='H1 strain')
